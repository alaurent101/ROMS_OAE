...
! Adapted from prsgrd32.h 2017/3/24
! This version uses the semi-prognostic method.

!
!svn $Id: prsgrd32.h 751 2015-01-07 22:56:36Z arango $
!***********************************************************************
...
! Modified to match format of rho_eos.F in 2020/11 ROMS 2021/3/1
     ...
     &                  GRID(ng) % z_w,                                 &
     &                  OCEAN(ng) % rho,                                &
! rho_clim added 2017/3/24, changed to rho_mean 2017/6/19
     &                  OCEAN(ng) % rho_mean,                           &
#ifdef ATM_PRESS
     &                  FORCES(ng) % Pair,                              &
#endif
...
! rho_clim added 2017/3/24, changed to rho_mean 2017/6/19
      real(r8), intent(in) :: rho_mean(LBi:,LBj:,:)
# ifdef ATM_PRESS
...
! rho_clim added 2017/3/24, changed to rho_mean 2017/6/19
      real(r8), intent(in) :: rho_mean(LBi:UBi,LBj:UBj,N(ng))
# ifdef ATM_PRESS
...
!
      DO j=JstrV-1,Jend
        DO k=1,N(ng)-1
          DO i=IstrU-1,Iend
            dR(i,k)=rho_mean(i,j,k+1)-rho_mean(i,j,k)
        ...
        DO i=IstrU-1,Iend
          ...
          cff2=0.5_r8*(rho_mean(i,j,N(ng))-rho_mean(i,j,N(ng)-1))*      &
     &         (z_w(i,j,N(ng))-z_r(i,j,N(ng)))*cff1
          P(i,j,N(ng))=GRho0*z_w(i,j,N(ng))+                            &
#ifdef ATM_PRESS
     &                 fac*(Pair(i,j)-OneAtm)+                          &
#endif
     &                 GRho*(rho_mean(i,j,N(ng))+cff2)*                 &
     &                 (z_w(i,j,N(ng))-z_r(i,j,N(ng)))
        END DO
        DO k=N(ng)-1,1,-1
          DO i=IstrU-1,Iend
            P(i,j,k)=P(i,j,k+1)+                                        &
     &               HalfGRho*((rho_mean(i,j,k+1)+rho_mean(i,j,k))*     &
     ...
     &                           (dZ(i,k+1)+dZ(i,k)))-                  &
     &                          (dZ(i,k+1)-dZ(i,k))*                    &
     &                          (rho_mean(i,j,k+1)-rho_mean(i,j,k)-     &
     &                           OneTwelfth*                            &
     &                           (dR(i,k+1)+dR(i,k)))))
          END DO
        END DO
...
#endif
            FC(i,j)=rho_mean(i,j,k)-rho_mean(i-1,j,k)
#ifdef MASKING
...
!
     ...
     &                     (P(i-1,j,k)-P(i,j,k)-                        &
     &                      HalfGRho*                                   &
     &                      ((rho_mean(i,j,k)+rho_mean(i-1,j,k))*       &
...
#endif
            FC(i,j)=rho_mean(i,j,k)-rho_mean(i,j-1,k)
#ifdef MASKING
...
!
     ...
     &                     (P(i,j-1,k)-P(i,j,k)-                        &
     &                      HalfGRho*                                   &
     &                      ((rho_mean(i,j,k)+rho_mean(i,j-1,k))*       &
